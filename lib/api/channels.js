...
ChannelsApi.prototype.device_list = function (pagination, callback) {
    var requestPath = REST_ENDPOINTS.DEVICE_LISTING;
    if (pagination && pagination.start) {
        requestPath += "?";

        if (pagination.limit)
            requestPath += "limit=" + pagination.limit + "&";

        requestPath += "start=" + pagination.start;
    }
